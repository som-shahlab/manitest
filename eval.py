...
import numpy as np
from utils import generate_prompts_with_injected_examples, logsumexp, manifest_score_sequences, manifest_generate_text, generation_multilabel_metric
from utils import log_model_tokenizer_config, log_task_config, log_classification_results, log_generations, log_generation_results
...
def run_task(func):
    def wrapper(manifest, 
        ...
        inputs_by_split: Dict[str, List[Tuple[str, str]]] = generate_prompts_with_injected_examples(dataset, prompt_template, output_column, output_template)
...
@run_task
def run_classification(manifest, 
                        task,
    ...
    for split in inputs_by_split.keys():
        ...
        batch_len: int = batch_size // terms_len # adjust batch size to account for multiple terms per class for each prompt
        for i in tqdm(range(0, len(inputs_by_split[split]), batch_len)):
            ...
            for prompt_idx, (prompt, y) in enumerate(batch):
                for label, terms in task.verbalizer.items():
                    for term in terms:
                        sequences.append({ 'prompt_idx' : prompt_idx, 'prompt' : prompt, 'y' : y, 'term' : term, 'y_hat' : label, 'logprob' : None })
            ...
            df = df.groupby(['prompt_idx', 'y_hat']).agg({
                'logprob' : logsumexp,
                'prompt' : 'first',
                'y' : 'first',
            }).reset_index()
            # Create generation of each sequence for interpretability
            generations: List[str] = [ x[0] for x in manifest_generate_text(manifest, df['prompt'].tolist(), max_new_tokens=32) ]
            df['generation'] = generations
            # Set prediction as most likely token
            df_preds = df.sort_values('logprob', ascending=False).drop_duplicates(['prompt_idx']).sort_values('prompt_idx')
...
def run_generation(manifest, 
    ...
    for split in inputs_by_split.keys():
        ...
        for i in tqdm(range(0, len(inputs_by_split[split]), batch_size)):
            ...
            generations: List[str] = [ x[0] for x in manifest_generate_text(manifest, prompts, max_new_tokens=max_new_tokens) ]
...
@run_task
def run_multilabel_classification(manifest, 
                                  task,
                                  inputs_by_split: Dict[str, List[Tuple[str, str]]] = {}, 
                                  batch_size: int = 10,
                                  path_to_output_dir: str = '',
