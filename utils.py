...
def manifest_score_sequences(manifest: Manifest, prompts_with_labels: List[Tuple[str, str]]) -> List[float]:
    try:
        results: Dict = requests.post(manifest.client.host + '/score_sequence_eleuther_lm_eval', 
                                      json={ 'prompts_with_labels' : prompts_with_labels }).json()
        scores = [ (x['label_prob']) for x in results ]
    except Exception as e:
        print(str(e))
        raise RuntimeError("No value returned from Manifest. This is probably a CUDA out-of-memory error. Try reducing the batch size.")
    return scores

def manifest_generate_text(manifest: Manifest, sequences: List[str], **kwargs) -> List[Tuple[str, float]]:
    try:
        results: Dict = requests.post(manifest.client.host + '/completions', json={ 'prompt' : sequences, **kwargs }).json()
...
def generate_prompts_with_injected_examples(dataset: DatasetDict, prompt_template: str, output_column: str, output_template: str) -> Dict[str, List[Tuple[str, str]]]:
    """Create prompts and inject dataset examples into prompt template.
    Assumes that the variable in the Jinja template has the same name as the attribute in each dataset example that we want to inject into the prompt.
        ...
        for example in dataset[split]:
            ...
            if isinstance(output_column, list):
                ...
                for i in range(len(output_column)):
                    expected_output.append(jinja_renderer.from_string(output_template).render(output=example[output_column[i]]))
            else:
...
def generate_text(model, 
                  input_ids,
                  max_new_tokens: int = 10) -> Dict[str, torch.Tensor]:
    """
    ...
    outputs = model.generate(input_ids, 
                                ...
                                # num_beams=num_beams,
                                output_scores=True, 
                                return_dict_in_generate=True) # dict, each element is: batch_size x seq_len x vocab_size
    return outputs
...
def get_predicted_token_logits(model, tokenizer, input_ids):
    ...
    if model.config.model_type in ['bert']:
        ...
        predicted_token_logits = token_logits[mask_token_idx, :] # batch_size x vocab_size (just the logits for the predicted/masked token)
        generation = '' # bert models don't generate text
        assert mask_token_idx.shape == (batch_size, predicted_token_logits.shape[1]), f"Mask token idx shape {mask_token_idx.shape} != predicted token logits shape {predicted_token_logits.shape}"
    elif model.config.model_type in ['gpt2', 't5', 'bart',]:
        outputs = generate_text(model, input_ids)
        # NOTE: outputs['scores'] starts after the prompt, while outputs['sequences'] includes the prompt in its sequence
        predicted_token_logits = outputs['scores'][0].detach().cpu().numpy() # (batch_size * num_beams * num_return_sequences) x vocab_size (just the logits for the predicted/masked token)
        predicted_sequences = outputs['sequences'].detach().cpu().numpy() # batch_size x max_length_of_output
        generation = tokenizer.decode(predicted_sequences[0][prompt_len:], skip_special_tokens=False) # get actual text generated by model
    else:
