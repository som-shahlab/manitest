...
def parse_yaml_config(path_to_dataset_config: str):
    ...
    REQUIRED_KEYS: List[str] = ['dataset_name', 'dataloader', 'dataloader_subset', 'is_classification', 'output_column', 'prompts', ]
    for key in REQUIRED_KEYS:
        if key not in yaml_dict:
            raise ValueError(f"You must specify the '{key}' attribute in your YAML config file @ {path_to_dataset_config}")
    
    # `is_classification` is a boolean
    if not isinstance(yaml_dict['is_classification'], bool):
        raise ValueError(f"You must specify the 'is_classification' attribute as a Boolean in your YAML config file @ {path_to_dataset_config}")
    ...
    for id, data in prompts.items():
        if 'template' not in data:
            raise ValueError(f"You must specify the 'template' attribute for prompt '{id}' in your YAML config file @ {path_to_dataset_config}")
...
def load_data(path_to_dataset_config: str, path_to_data_dir: str) -> Tuple[DatasetDict, List[Task], bool]:
    ...
    for id, data in prompts.items():
        ...
        if is_classification:
            ...
            verbalizer = { k: v for d in data['verbalizer'] for k, v in d.items() } # [key] = class, [value] = list of terms mapping to this class
