# coding=utf-8
# Copyright 2022 The HuggingFace Datasets Authors and the current dataset script contributor.
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

"""
The files comprising this dataset must be on the users local machine in a single directory that is
passed to `datasets.load_datset` via the `data_dir` kwarg. This loader script will read the archive
files directly (i.e. the user should not uncompress, untar or unzip any of the files). For example,
if `data_dir` is `"radnli"` it should contain the following files:
radnli
├── /local-scratch/nigam/projects/clinical_llm/data/pheno_anno/phenotype-annotations-for-patient-notes-in-the-mimic-iii-database-1.20.03.zip
"""

import json
import os
from typing import Dict, List, Tuple
import pandas as pd
import numpy as np

import datasets

from .bigbiohub import text_features
from .bigbiohub import BigBioConfig
from .bigbiohub import Tasks


_LANGUAGES = ["English"]
_PUBMED = False
_LOCAL = True
_CITATION = """\
"""

_DATASETNAME = "phenoanno"
_DISPLAYNAME = "PhenoAnno"

_DESCRIPTION = """\
"""


_HOMEPAGE = ""

_LICENSE = "PHYSIONET_LICENSE_1p5"

_URLS = {}

_SUPPORTED_TASKS = [Tasks.TEXTUAL_ENTAILMENT]

_SOURCE_VERSION = "1.0.0"
_BIGBIO_VERSION = "1.0.0"


class PhenoAnnoDataset(datasets.GeneratorBasedBuilder):
    """phenoanno"""

    SOURCE_VERSION = datasets.Version(_SOURCE_VERSION)
    BIGBIO_VERSION = datasets.Version(_BIGBIO_VERSION)

    BUILDER_CONFIGS = [
        BigBioConfig(
            name="phenoanno_source",
            version=SOURCE_VERSION,
            description="phenoanno source schema",
            schema="source",
            subset_id="phenoanno",
        ),
        BigBioConfig(
            name="phenoanno_bigbio_txtclass",
            version=BIGBIO_VERSION,
            description="phenoanno BigBio schema",
            schema="bigbio_txtclass",
            subset_id="phenoanno",
        ),
    ]

    DEFAULT_CONFIG_NAME = "phenoanno_source"

    def _info(self) -> datasets.DatasetInfo:

        if self.config.schema == "source":
            features = datasets.Features(
                {
                   
                }
            )

        elif self.config.schema == "bigbio_txtclass":
            features = text_features

        return datasets.DatasetInfo(
            description=_DESCRIPTION,
            features=features,
            homepage=_HOMEPAGE,
            license=str(_LICENSE),
            citation=_CITATION,
        )

    def _split_generators(self, dl_manager) -> List[datasets.SplitGenerator]:
        #print(f"split Generator {self.config.data_dir}")
        
        if self.config.data_dir is None:
            raise ValueError(
                "This is a local dataset. Please pass the data_dir kwarg to load_dataset."
            )
        else:
            extract_dir = dl_manager.extract(
                os.path.join(
                    self.config.data_dir,
                    "phenotype-annotations-for-patient-notes-in-the-mimic-iii-database-self_split.zip",
                )
            )
            # print(f"Extract {extract_dir}")
            # print(os.listdir(extract_dir))
            # exit()
            data_dir = extract_dir
            # data_dir = os.path.join(
            #     extract_dir,
            #     "phenotype-annotations-for-patient-notes-in-the-mimic-iii-database-self_split",
            # )

        return [
            datasets.SplitGenerator(
                name=datasets.Split.TRAIN,
                gen_kwargs={
                    "filepath": os.path.join(data_dir, "train.csv"),
                    "split": "train",
                    "labels": ['cohort',
                                'Obesity',
                                'Non.Adherence',
                                'Developmental.Delay.Retardation',
                                'Advanced.Heart.Disease',
                                'Advanced.Lung.Disease',
                                'Schizophrenia.and.other.Psychiatric.Disorders',
                                'Alcohol.Abuse',
                                'Other.Substance.Abuse',
                                'Chronic.Pain.Fibromyalgia',
                                'Chronic.Neurological.Dystrophies',
                                'Advanced.Cancer',
                                'Depression',
                                'Dementia',
                                'Unsure',]
                },
            ),
            datasets.SplitGenerator(
                name=datasets.Split.TEST,
                gen_kwargs={
                    "filepath": os.path.join(data_dir, "test.csv"),
                    "split": "test",
                    "labels": ['cohort',
                                'Obesity',
                                'Non.Adherence',
                                'Developmental.Delay.Retardation',
                                'Advanced.Heart.Disease',
                                'Advanced.Lung.Disease',
                                'Schizophrenia.and.other.Psychiatric.Disorders',
                                'Alcohol.Abuse',
                                'Other.Substance.Abuse',
                                'Chronic.Pain.Fibromyalgia',
                                'Chronic.Neurological.Dystrophies',
                                'Advanced.Cancer',
                                'Depression',
                                'Dementia',
                                'Unsure',]
                },
            ),
            datasets.SplitGenerator(
                name=datasets.Split.VALIDATION,
                gen_kwargs={
                    "filepath": os.path.join(data_dir, "val.csv"),
                    "split": "dev",
                    "labels": ['cohort',
                                'Obesity',
                                'Non.Adherence',
                                'Developmental.Delay.Retardation',
                                'Advanced.Heart.Disease',
                                'Advanced.Lung.Disease',
                                'Schizophrenia.and.other.Psychiatric.Disorders',
                                'Alcohol.Abuse',
                                'Other.Substance.Abuse',
                                'Chronic.Pain.Fibromyalgia',
                                'Chronic.Neurological.Dystrophies',
                                'Advanced.Cancer',
                                'Depression',
                                'Dementia',
                                'Unsure',]
                },
            ),
        ]

    def _generate_examples(self, filepath, split: str, labels: List[str]) -> Tuple[int, Dict]:
        with open(filepath, "r") as f:
            if self.config.schema == "source":
                for line in f:
                    json_line = json.loads(line)
                    yield json_line["pair_id"], json_line

            elif self.config.schema == "bigbio_txtclass":
                
                df = pd.read_csv(filepath)
                for index, row in df.iterrows():
                    label_vals = np.array([row[label] for label in labels])
                    yield row.iloc[0], {
                        'id': int(index),
                        "document_id": int(index),
                        "text": row['text'],
                        "labels": label_vals,
                    } 
                    




                
if __name__ == "__main__":
    dataset = datasets.load_dataset(__file__, 
                                    data_dir='/local-scratch/nigam/projects/clinical_llm/data/pheno_anno',
                                    name='phenoanno_bigbio_txtclass')
    print(dataset)