...
def main(args):
    ...
    try:
        _ = requests.get(args.manifest_url)
    ...
    for task in tasks:   
        if not is_multilabel and is_classification:
            # Classification task
            result, metric = run_classification(manifest,
                                                task,
                                                dataset,
            ...
            # Generation task
            result, metric = run_generation(manifest,
                                            task,                                            
                                            dataset,
                                            batch_size=args.batch_size,
                                            path_to_output_dir=args.path_to_output_dir,
            ...
            result, metric = run_multilabel_classification(manifest,
                                                           task,
                                                           dataset,
                                                           batch_size=args.batch_size,
                                                           path_to_output_dir=args.path_to_output_dir,
    ...
    # Save metrics
    path_to_metrics_file: str = os.path.join(args.path_to_output_dir, f'metrics.json')
    ...
    if is_multilabel:
        ...
        for label in tasks[0].verbalizer.keys():
            ...
            print(f'---------- {label} ---------------')
            print('------------------------------------------------------')
            print("Average: ", round(sum([ r['test']['multilabel_score'][label]['accuracy'] for r in metrics.values()]) / len(metrics), 3))
        ...
        with open(os.path.join(args.path_to_output_dir, f'metrics.txt'), 'w') as f:
            f.write('------------------------------------------------------\n')
...
if __name__ == "__main__":
    ...
    # Required
    parser.add_argument(
        "--manifest_url", type=str, help="Host + port where Manifest server is running, e.g. 'localhost:5000'", required=True,
    )
    parser.add_argument(
        "--path_to_dataset_config", type=str, help="Path to configuration YAML for this dataset", required=True,
    )
    parser.add_argument(
        "--path_to_dataset_dir", type=str, help="Path to DIRECTORY containing your dataset", required=True,
    )
    parser.add_argument(
        "--path_to_output_dir", type=str, help="Path to DIRECTORY to output logs / results / metrics", required=True,
    )
    
    # Optional
    parser.add_argument(
        "--batch_size", type=int, help="Number of examples per batch", default=10,
    )
    parser.add_argument(
        "--is_show_log", action=argparse.BooleanOptionalAction, help="If specified, print logs to console", default=False,
    )
    parser.add_argument(
        "--max_new_tokens", type=int, help="Max new generation token length", default=512,
    )
    
    # Decoder args (optional)
    parser.add_argument(
        "--do_sample", type=bool, help="Sampling while generating from model", default=True,
    )
    parser.add_argument(
        "--early_stopping", type=bool, help="Stop early while generating", default=False,
    )
    parser.add_argument(
        "--num_return_sequences", type=int, help="Number of sequences to generate", default=1,
    )
    parser.add_argument(
        "--num_beams", type=int, help="Number of beams to search", default=5,
    )
    parser.add_argument(
        "--top_p", type=float, help="For nucleus sampling", default=0.9,
    )
